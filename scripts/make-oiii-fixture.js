/**
 * Copy a few SII lights/flats and rewrite FILTER header + filename to OIII
 * so the same night has a third filter for ingest-all testing.
 */
const fs = require('fs');
...
async function copyAsFilter(srcPath, destPath, filterCode) {
  const buf = await fsp.readFile(srcPath);
  const patched = patchFilterCard(buf, filterCode);
...
async function main() {
  ...
  for (const name of lights) {
    const destName = name.replace(/_S_/g, '_O_');
    const dest = path.join(LIGHT_DIR, destName);
    if (fs.existsSync(dest)) {
      console.log('skip light', destName);
      continue;
    }
    await copyAsFilter(path.join(LIGHT_DIR, name), dest, 'O');
    console.log('OIII light', destName);
  }
  for (const name of flats) {
    ...
    await copyAsFilter(path.join(FLAT_DIR, name), dest, 'O');
    console.log('OIII flat', destName);
  }
  console.log('done');
}
